<div class="form-model search">
    ...
    <form method="GET" action="" style="margin-bottom: 20px;">
        ...
        <div class="form-group">
            ...
            <input type="text" name="busca" placeholder="Pesquisar por nome" value="<?= isset($_GET['busca']) ? htmlspecialchars($_GET['busca']) : '' ?>">
        </div>
...
<div class="list-model">
    ...
    $condicoes = "WHERE assuntos.ativo = 1";

    if (!empty($busca)) {
        $buscaSegura = mysqli_real_escape_string($conexao, $busca);
        $condicoes .= " AND (assuntos.nome LIKE '%$buscaSegura%')";
    }
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($dados = mysqli_fetch_assoc($result)) {
            ...
            echo "<ul class='list-items'>
                    <li>{$dados['nome']}</li>
                    <li>
                        <form method='POST' action='editar_assunto.php'>
...
<script>
    function deletar(botao) {
        if (confirm('Você tem certeza que deseja excluir permanentemente este assunto?')) {
            const form = botao.closest("form");
            const formData = new FormData(form);

            fetch("../api/excluir_assunto.php", {
                method: "POST",
                body: formData
            })
            .then(res => res.json())
